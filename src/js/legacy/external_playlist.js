...
(function (CyTube_Playlist) {
  ...
  const options = Object.assign(
    ...
    window[CHANNEL.name].modulesOptions
      ? window[CHANNEL.name].modulesOptions.playlist
      : undefined
  );
  ({
    start: function () {
  ...
  ({
    start: function () {
      ...
      $("<style>")
        .prop("id", "thumbnailer")
        .text(
          ".playlist-thumbnail { max-height: 180px; max-width: 240px; border-radius: 4px; }"
        )
        .appendTo("head");
      $("#queue").on("mouseleave", () => {
        this.trimOrphans();
    ...
    getThumbnail: function (target) {
      ...
      switch (type) {
        ...
        case "yt":
          ...
          break;
        case "dm":
          url = "https://www.dailymotion.com/thumbnail/video/__id".replace(
            /__id/,
            id
          );
          break;
        ...
        .click(function () {
          ...
          if (isMarked) {
            ...
            $("#messagebuffer").trigger(
              "whisper",
              `You have marked a video for playing notification.`
            );
          }
        });
      if (usingGroup) {
  ...
  void (() => {
    ...
    HTMLCollection.prototype.each = Array.prototype.each = NodeList.prototype.each = function (
      func,
      _this
    ) {
      var i = -1,
    ...
    document.head.insertAdjacentHTML(
      "beforeEnd",
      "<style>#queue li:hover .qe_time:before { content: attr(data-timeleft); }</style>"
    );
    ...
    function calculateRemainingTime() {
      ...
      var active,
        ...
        total = 0;
      var currentTime =
        m && "playtime" in m.dataset && m.dataset["playtime"] >= 0
          ? m.dataset["playtime"]
          : 0;
      ...
      q.each(function injectDOM() {
        ...
        if (!active) {
          if (this.classList.contains("queue_active")) {
            ...
            this.querySelector(".qe_time").dataset.timeleft =
              "Time left: " + secondsToTimeStr(total) + " | ";
          } else cycle.push(this);
          return;
        } else {
          this.querySelector(".qe_time").dataset.timeleft =
            "Time till: " + secondsToTimeStr(total) + " | ";
  ...
  ({
    ...
    update: function (data) {
      ...
      if (
        this.history.length &&
        newEntry.link === this.history[this.history.length - 1]["link"]
      ) {
        return;
      }
      this.history.push(newEntry);
      this.history = this.history.slice(
        this.history.length > this.maxhist
          ? this.history.length - this.maxhist
          : 0,
        this.history.length
      );
    ...
    start: function () {
      ...
      this.history = JSON.parse(
        localStorage.getItem(`${CHANNEL.name}_RecentMedia`)
      );
      var self = this;
      var pane = $("<div>")
        .attr("id", "recentmedia")
  ...
  void (() => {
    ...
    $("<button>")
      ...
      .addClass("btn btn-sm")
      .addClass(
        localStorage.getItem(`${CHANNEL.name}_HidePlaylist`) === "on"
          ? "btn-warning"
          : "btn-default"
      )
  ...
  void (() => {
    ...
    const minrank = Math.min(
      CHANNEL.perms.oplaylistdelete,
      CHANNEL.perms.playlistdelete,
      CHANNEL.perms.oplaylistjump,
      CHANNEL.perms.playlistjump,
      CHANNEL.perms.oplaylistnext,
      CHANNEL.perms.playlistnext
    );
    ...
    $("<button>")
      ...
      .addClass("btn btn-sm")
      .addClass(
        localStorage.getItem(`${CHANNEL.name}_CollapsePlaylist`) === "on"
          ? "btn-warning"
          : "btn-default"
      )
  ...
  void (() => {
    ...
    $("<button/>")
      ...
      .html(
        "<span class='glyphicon glyphicon-hd-video'></span><strong> " +
          current +
