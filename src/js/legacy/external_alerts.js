...
(function (CyTube_Notifications) {
  ...
  if (typeof Storage === "undefined") {
    ...
    console.error(
      "[XaeTube: Audio Notifier]",
      "Playlist parser unavailable. Aborting load."
    );
    return;
    return;
  } else {
  ...
  if (typeof window[CHANNEL.name].audioLibrary === "undefined") {
    console.warn(
      "[XaeTube: Audio Notifier]",
      "WARNING: Audio library module not loaded."
    );
  }
  if (!$("#customSettingsStaging").length) {
    console.warn(
      "[XaeTube: Audio Notifier]",
      "WARNING: Settings module not loaded."
    );
  }
  const AudioNotifier = function () {
    ...
    this.choices = Object.assign(
      ...
      window[CHANNEL.name].audioLibrary
        ? window[CHANNEL.name].audioLibrary.squees
        : undefined
    );
    this.handler = {
      Squee: function (data) {
        ...
        if (!CHANNEL.opts.chat_antiflood) {
          console.info(
            "[XaeTube: Audio Notifier]",
            "User ping ignored: Chat throttle off."
          );
          return;
        }
      ...
      Priv: function (data) {
        ...
        $(
          "div.chat-msg-\\\\\\$server\\\\\\$:contains(Direct Message Notification)"
        ).remove();
        $("#messagebuffer").trigger(
          "whisper",
          `Direct Message Notification: ${data.username}`
        );
      }.bind(this),
      Video: function (data) {
        var addedby;
        ...
        $(
          "div.chat-msg-\\\\\\$server\\\\\\$:contains(Video Notification)"
        ).remove();
        $("#messagebuffer").trigger(
          "whisper",
          "Video Notification: Your video is now playing!"
        );
      }.bind(this),
      Marked: function (uid) {
        ...
        $("#messagebuffer").trigger(
          "whisper",
          "Video Notification: A video you marked is now playing!"
        );
  ...
  Object.assign(AudioNotifier.prototype, {
    pushNoticeChange: function (change) {
      ...
      this[type].audio = $("<audio>")
        ...
        .append(
          $("<source>").attr("src", this[type].file).attr("type", "audio/ogg")
        );
      if (!silent) {
        this[type].audio[0].play();
        $(
          "div.chat-msg-\\\\\\$server\\\\\\$:contains(" +
            this.typeNames[type] +
            " Notification)"
        ).remove();
        $("#messagebuffer").trigger(
          "whisper",
          this.typeNames[type] + " Notification Changed to: " + id
        );
      }
    },
    ...
    toggle: function (type) {
      ...
      localStorage.setItem(
        `${CHANNEL.name}_AudioNotice${type}Toggle`,
        +this[type].toggleState
      );
    ...
    createControls: function (types) {
      ...
      while (types.length) {
        ...
        var dropdown = $("<button/>")
          ...
          .html(
            "<span class='glyphicon glyphicon-music'></span> Sound <span class='caret'></span>"
          )
          .appendTo(sounds);
    ...
    initialize: function () {
      ...
      if (
        ...
        window[CHANNEL.name].modulesOptions.audioNotice
      ) {
        this.choices = Object.assign(
          this.choices,
          window[CHANNEL.name].modulesOptions.audioNotice.choices
        );
        var notices = Object.keys(
          window[CHANNEL.name].modulesOptions.audioNotice.notices
        );
        for (var i = notices.length - 1; i >= 0; i--) {
          this[notices[i]]["id"] =
            window[CHANNEL.name].modulesOptions.audioNotice.notices[notices[i]];
        }
      }
      var types = Object.keys(this.typeNames);
      while (types.length) {
        var type = types.shift();
        var toggle = localStorage.getItem(
          `${CHANNEL.name}_AudioNotice${type}Toggle`
        );
        var id = localStorage.getItem(`${CHANNEL.name}_AudioNotice${type}ID`);
        var volume = localStorage.getItem(
          `${CHANNEL.name}_AudioNotice${type}Volume`
        );
        if (toggle) {
        ...
        if (volume) {
          this[type].volume = parseInt(volume) / 100 || 0.6;
        }
        this.pushNoticeChange({ type: type, id: this[type].id, silent: true });
      }
      this.createToggles();
